...
class TestVLogic(unittest.TestCase):
    ...
    def test_v_refresh_current_vim(self, mock_run, mock_check_output):
        ...
        # Mock tmux list-panes and then ps -t
        mock_check_output.side_effect = ["%1:1:/dev/ttys001:zsh\n%2:0:/dev/ttys002:vim", "vim"]
    ...
    @patch('subprocess.check_output')
    @patch('subprocess.run')
    def test_v_resume_background_nvim(self, mock_run, mock_check_output):
        """Test 'fg' is sent when nvim is in background."""
        # target pane (%2) is running zsh, but has nvim on TTY
        mock_check_output.side_effect = ["%1:1:/dev/ttys001:zsh\n%2:0:/dev/ttys002:zsh", "zsh\nnvim"]
        
        refresh_editor('test.py')
        
        calls = [c.args[0] for c in mock_run.call_args_list]
        self.assertIn(['tmux', 'send-keys', '-t', '%2', 'fg %nvim', 'Enter'], calls)
        self.assertIn(['tmux', 'send-keys', '-t', '%2', 'Escape', 'Escape'], calls)

    @patch('subprocess.check_output')
    def test_v_no_other_pane(self, mock_check_output):
        """Test fails when no other pane is found."""
        mock_check_output.return_value = "%1:1:/dev/ttys001:zsh"
